...
export const queryPineconeDatabase: RequestHandler = async (
  ...
  try {
    ...
    // add some code here to query the Pinecone database
    const index = pc.index('movies');

    const result = await index.query<MovieMetadata>({
      topK: 5,
      vector: embedding,
      includeMetadata: true,
      filter: {
        year: {
          $gte: parseInt(res.locals.startYear),
          $lte: parseInt(res.locals.endYear),
        },
      },
    });
    ...
    const apiError: ServerError = {
      log: `queryPineconeDatabase: Error: Database query failed. error: ${err}`,
      status: 500,
