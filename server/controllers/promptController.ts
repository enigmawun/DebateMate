...
export const customizePrompts: RequestHandler = async (_req, res, next) => {
  ...
  const role = `
  You are a ${res.locals.role} in a back and forth dialogue with a user, where each person gets 3 rounds to make their argument total.
  You are representing the AI side in this debate. The debate conversation will be provided to you in the user prompt.
  `;

  const context = `
  You are currently in the round ${res.locals.round} of the debate with a human (user). You are representing the AI side.
  This is the existing debate between the user(human) and AI(you) so far:
  ${argumentString}
  `;

  const instructions = `You are crafting a three-part argument ${ai_side} the idea that ${parsedTopic}.

  Please follow these instructions carefully:
    - You will be sending one part of your argument at a time.
    - For the first part of your argument, you will be crafting an initial argument about the topic and you will be expecting a rebuttal. if the
      user has made a claim or comment in the last round, you should first briefly respond to it before giving your argument.
    ...
    - You will be crafting each section of argument in response to the user's rebuttal of your previous claim. If user has made a claim or rebuttal, you should repond
      with a direct and strong assertive counter argument. If user has a point that is particularly strong, you should acknowledge it and provide a counter argument.
      If user has a point that is particularly weak, you should point out the weakness, shame the user, and provide a super strong counter argument.
    - Be as conversational as possible in your argument - be as personal and natural as you can.
      You are acting as a person (a conversation partner) in a debate. Do not be too formal and literal in your arguments.
  `;
  ...
  `;
  const systemContent = role + instructions + responseFormat;
  const userContent = context;

  res.locals.systemContent = systemContent;
  res.locals.userContent = userContent;
...
export const customizeEvaluationPrompts: RequestHandler = async (
  ...
  The ai side is ${ai_side} the topic, and the user side is on another side. You will call the user "you" throughout the assessment, like you are talking to them in person.
  `;
...
Please follow the assessment rubrics below, be very strict in your assessment.
If the user sucks at debating, you should tell them they suck. If the AI sucks at debating, you should tell them they suck.
...
export const customizeHintPrompts: RequestHandler = async (_req, res, next) => {
